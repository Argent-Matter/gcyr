...
public class DysonSystemControllerMachine extends WorkableElectricMultiblockMachine {
    ...
    protected @Nullable GTRecipe getRealRecipe(GTRecipe recipe) {
        ...
        if (recipe.data.contains("gcyr:repair_dyson_sphere")) {
            IDysonSystem system = GCYRCapabilityHelper.getDysonSystem((ServerLevel) this.getLevel());
            if (system != null && system.isDysonSphereActive() && (!system.activeDysonSphere().isNeedsMaintenance() ||
                    !this.getPos().equals(system.activeDysonSphere().getControllerPos())))
                return null;
        }
    ...
    public boolean onWorking() {
        ...
        BlockPos pos = this.getPos().mutable().move(backFacing, 4).move(rightFacing, 1)
                .move(0, 7 + 256 /* pre offset up by half distance */, 0).immutable();
        for (LivingEntity entity : level.getEntitiesOfClass(LivingEntity.class,
                AABB.ofSize(Vec3.atCenterOf(pos), 3, 512, 3), EntitySelector.LIVING_ENTITY_STILL_ALIVE)) {
    ...
    public void afterWorking() {
        ...
        if (recipe.getInputContents(ItemRecipeCapability.CAP).isEmpty()) return; // assume the recipe is a dyson launch
                                                                                 // or repair if it has item inputs.

        IDysonSystem system = GCYRCapabilityHelper.getDysonSystem((ServerLevel) this.getLevel());
        if (system == null) return;
        if (!system.isDysonSphereActive() && recipe.data.contains("gcyr:launch_dyson_sphere")) {
            system.addDysonSphere(this.getPos());
        } else if (system.isDysonSphereActive() && recipe.data.contains("gcyr:repair_dyson_sphere") &&
                system.activeDysonSphere().getControllerPos().equals(this.getPos())) {
        ...
        if (!isRemote()) {
            ...
            if (system.isDysonSphereActive()) {
                if (system.activeDysonSphere().isNeedsMaintenance() && !system.activeDysonSphere().isCollapsed()) {
                    textList.add(Component.translatable("menu.gcyr.dyson_sphere.needs_maintenance")
                            .withStyle(this.getOffsetTimer() % 10 >= 5 ? ChatFormatting.RED : ChatFormatting.DARK_RED));
                    textList.add(Component.translatable("menu.gcyr.dyson_sphere.time_since_needed_maintenance",
                            system.activeDysonSphere().getTimeNeededMaintenance()));
                    textList.add(Component.translatable("menu.gcyr.dyson_sphere.implosion_chance",
                            system.activeDysonSphere().getCollapseChance() * 100.0f));
