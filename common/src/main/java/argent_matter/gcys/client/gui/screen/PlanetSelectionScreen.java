...
public class PlanetSelectionScreen extends Screen implements MenuAccess<PlanetSelectionMenu> {
    ...
    private void renderBackground(PoseStack poseStack, int mouseX, int mouseY, float delta) {
        ...
        for (SolarSystem system : GregicalitySpaceClient.solarSystems) {
        ...
        for (PlanetRing ring : GregicalitySpaceClient.planetRings) {
        ...
        if (currentPage == 1) {
            GregicalitySpaceClient.galaxies.stream().filter(g -> g.galaxy().equals(this.currentCategory.id()))
                .findFirst()
        ...
        if (currentPage == 1 && GregicalitySpaceClient.galaxies.size() <= 1) {
    ...
    private int getPage() {
        ...
        GregicalitySpace.LOGGER.warn("Invalid page!");
        return 0;
    }
    ...
    public int getColumn(Category category) {
        ...
        if (Category.GALAXY_CATEGORY.equals(category.parent()) && GregicalitySpaceClient.galaxies.size() <= 1) {
