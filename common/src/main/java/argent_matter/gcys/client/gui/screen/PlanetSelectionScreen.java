...
public class PlanetSelectionScreen extends Screen implements MenuAccess<PlanetSelectionMenu> {
    ...
    private void renderBackground(PoseStack poseStack, int mouseX, int mouseY, float delta) {
        ...
        for (SolarSystem system : GCySClient.solarSystems) {
        ...
        for (PlanetRing ring : GCySClient.planetRings) {
        ...
        if (currentPage == 1 && GCySClient.galaxies.size() <= 1) {
    ...
    public void createGalaxyButton(Category galaxyCategory) {
        ...
        Galaxy galaxy = GCySClient.galaxies.stream().filter(g -> g.galaxy().equals(galaxyCategory.id())).findFirst().orElse(null);
    ...
    public void createSolarSystemButton(Category solarSystemCategory) {
        ...
        SolarSystem solarSystem = GCySClient.solarSystems.stream().filter(g -> g.solarSystem().equals(solarSystemCategory.id())).findFirst().orElse(null);
