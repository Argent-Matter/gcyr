...
import screret.gcys.common.config.GcysConfigHolder;
...
public class LaserSatellite extends Satellite {
    ...
    public void tickSatellite(Level level) {
        ...
        if (isMining) {
            if (level.getGameTime() % GcysConfigHolder.INSTANCE.machines.laserSatelliteMiningTickStep == 0) {
                ...
                entities.forEach(entity -> entity.hurt(DamageSources.getRadioactiveDamage(), GcysConfigHolder.INSTANCE.machines.laserSatelliteDamagePerTickStep));
            }
        }
